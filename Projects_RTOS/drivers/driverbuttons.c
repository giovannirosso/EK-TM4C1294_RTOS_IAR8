...
void ButtonInit(uint8_t buttons)
{
  ...
  {
    GPIOPinTypeGPIOInput(GPIO_PORTJ_BASE, (GPIO_PIN_0 | GPIO_PIN_1));                                       // push-button SW2 como entrada
    GPIOPadConfigSet(GPIO_PORTJ_BASE, (GPIO_PIN_0 | GPIO_PIN_1), GPIO_STRENGTH_2MA, GPIO_PIN_TYPE_STD_WPU); // habilita pull-up
  }                                                                                                         // if
} // ButtonInit
...
void ButtonIntDisable(uint8_t buttons)
{
  ...
  if (buttons & (USW1 | USW2))
  {
    GPIOIntDisable(GPIO_PORTJ_BASE, (GPIO_INT_PIN_0 | GPIO_INT_PIN_1));
  }
} // ButtonIntDisable
